...
function registerUsers(AltoRouter $router): void {
    ...
    $router->map('POST', "/users/create", 'UsersController#create'); // 회원 가입
    $router->map('PUT', "/users/update/[a:account]", 'UsersController#update'); // 회원 정보 수정
    $router->map('DELETE', "/users/delete/[a:account]", 'UsersController#delete'); // 회원 탈퇴
