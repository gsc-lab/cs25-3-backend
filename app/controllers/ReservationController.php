...
class ReservationController {
    ...
    // 로그인한 사용자 정보를 저장하기 위한 클래스 프로퍼티
    private int $user_id; // 로그인한 사용자의 ID
    private string $role; // 로그인한 사용자의 권한(예: client / designer / manager)
    public function __construct(){
        
        // 세션에 저장된 로그인 정보에서 user_id 값을 가져와 프로퍼티에 설정
        $this->user_id = $_SESSION['user']['user_id'];
        // 세션에 저장된 로그인 정보에서 role(권한) 값을 가져와 프로퍼티에 설정
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                if (!isset($reservations[$rid])) {
                    $reservations[$rid] = [
                        ...
                        'services'       => [],     // service는 예러게 있기 때문에 리스터형으로
                        'total_price'    => 0
                    ];
                }
    ...
    public function update(string $reservation_id):void {
        ...
        try {
            ...
            json_response(ErrorHandler::server($e, '[reservation_update]'),500);
        }
